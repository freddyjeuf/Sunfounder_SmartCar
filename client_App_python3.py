# Edited to python3 by JF - 06.2020
# tcpCliSock.send encoded into bytes for transmission puposes
# tkinter, not Tkinter
#---------------------------------------------
...
def forward_fun(event):
	print ('forward')
	f = 'forward'
	tcpCliSock.send(f.encode())

def backward_fun(event):
	print ('backward')
	b = 'backward'
	tcpCliSock.send(b.encode())

def left_fun(event):
	print ('left')
	l = 'left'
	tcpCliSock.send(l.encode())
...
def home_fun(event):
	print ('home')
	h = 'home'
	tcpCliSock.send(h.encode())

def x_increase(event):
	print ('x+')
	xp = 'x+'
	tcpCliSock.send(xp.encode())

def x_decrease(event):
	print ('x-')
	xm = 'x-'
	tcpCliSock.send(xm.encode())

def y_increase(event):
	print ('y+')
	yp = 'y+'
	tcpCliSock.send(yp.encode())

def y_decrease(event):
	print ('y-')
	ym = 'y-'
	tcpCliSock.send(ym.encode())

def xy_home(event):
	print ('xy_home')
	xyh = 'xy_home'
	tcpCliSock.send(xyh.encode())
